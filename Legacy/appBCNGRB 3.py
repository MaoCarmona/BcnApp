...
def EnvioARES(ItpInfo, IarrParametros, IUsrAuditoria):
    ...
    if vRespuesta == True:        
        ...
        if ItpInfo == 'COSTOS':        
            ...
            vSqlAux = vSqlAux.replace('[dtConsultaIni]', str(vFechaAuxI))
            vSqlAux = vSqlAux.replace('[dtConsultaFin]', str(vFechaAuxF))
            vSqlAux = vSqlAux.replace('[Filtro]', 'AND ml.txProcesamiento IS NULL')    
            # print(vSqlAux)        
            vtxJSON = IarrParametros["txMovLogistico"]
        ...
        if len(vDatosRS):
            ...
            for I in range(len(vDatosRS)):
                ...
                if ItpInfo == 'INVENTARIOS':    
                    ...
                    vTransaccionOrigenAux = vTransaccionOrigen + " " + str(vdtContable) + " "+ str(vDatosRS[I]["idObjCosto"]) + ' - '+str(vDatosRS[I]["ObjEstadistico"]) 
                elif ItpInfo == 'MOVIMIENTOS':
...
def CargarConfiguracionXML(InmRutaTrabajo, InmArcXML):
    ...
    try:        
        ...
        vLstParametros =  {"txMovCostos": vInfoMovCostos.text, "txMovLogistico": vInfoDMovLogistico.text, "InfoURLARES": arrTokenARES, "InfoDBROMSS": tkInfoDBROMSS, "InfoDBAORA": tkInfoDBAORA, "InfoDBAORAQAS": tkInfoDBAORAQAS, "InfoDBBCN": tkInfoDBBCN, "xQuerys": xmlQuerys}
...
def getVisualizarInfoHTML(ItpInfo, IarrParametros, ItxRutaTrabajo):
    ...
    if ItpInfo == "InfOPerativo":
        ...
        if vIdOpcion[:2] == "01":
            ...
            vtmpInvOperativo = vtmpInvOperativo.replace("{FechaIni}", dtFechaIni)
            vtmpInvOperativo = vtmpInvOperativo.replace("{FechaFin}", dtFechaFin) 
        elif vIdOpcion[:2] in ("06", "07", "08"): # Opc Dif Balance
            vEntrar = False            
            messagebox.showinfo(message="Esta opción no esta disponible:\n\n" + vIdOpcion, title = vFuente + " .::AppBCN")
        else:
            vtmpInvOperativo = vtmpInvOperativo.replace("{FechaIni}", dtFechaIni)
    ...
    if ItpInfo == "InfOPerativo":
        ...
        if vIdOpcion[:2] in ("01", "04"):        
            ...
            if vRespuesta == True:
                ...
                vEntrar = False
        # Movimientos y Flujos de AORA y Movimientos de ROMSS
        elif vIdOpcion[:2] in ("02", "03", "05", "07"):
            vTagQuery = "MOVOPERAORA"
            vTag = "MOVIMIENTOS OPERATIVOS"
            vRespuesta = messagebox.askokcancel(message="¿Esta seguro que desea ejecutar la información del sistema " + vIdOpcion[3:] + " ? \n\nPeriodo: \nDesde: "+ str(dtFechaIni) + " Hasta: "+ str(dtFechaFin), title=vFuente + " .::AppBCN")                    
            if vRespuesta == False:
        ...
        if vIdOpcion[:2] in ("01", "06"):
            ...
            if vRespuesta == False:
                vEntrar = False
        elif vIdOpcion[:2] in ("07", "08"): # Opc Dif Balance
            vEntrar = False            
    ...
    if ItpInfo == "InfOPerativo":
        ...
        if vIdOpcion[:2] == "01":
            ...
            if vEntrar == True:                            
                ...
                varrItems = getConvertirXML(vDatosRS, vTag)            
                vEntrar = True        
        elif vIdOpcion[:2] in ("02", "08"):        
            vTagQuery = "MOVCONSBCN"
            vTag = "Movimientos"                     
            if vIdOpcion[:2] == "08": # Opc Dif Balance
...
if __name__ == '__main__':
    ...
    vdtContableIni = datetime.now().strftime("%Y-") + "06-01"
    vdtContableFin = datetime.now().strftime("%Y-") + "06-01"
    ...
    if vboLeerRuta == True:
        ...
        vLstOpciones = ["01. BCN: Inventarios", "02. BCN: Movimientos", "03. BCN: Balance ALMACEN", "04. BCN: Balance POOL", "05. BCN: Balance UNIDAD DE PROCESO", "06. BCN: Foto Inventario", "07. BCN: Aplicar Regla de Balance", "08. BCN: Diferencia Balance"]
        ...
        vLstOpciones = ["01. BCN: Inventario Logistico", "02. BCN: Movimiento Logistico", "03. BCN: Movimiento de Costos", "04. ARES: Revisar Procesamiento"]
        ...
        botonEnvioWSARESInfo = ttk.Button(text="Ejecutar", command = lambda: getIntegrarInfo("InfEnviosARES", varrParametros, vRutaTrabajo))
        ...
        botonEnvioWSARESInfoExcel = ttk.Button(text="Rev. Excel", command = lambda: getVisualizarInfoExcel("InfEnviosARES", varrParametros, vRutaTrabajo))
